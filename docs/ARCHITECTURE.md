...
downloads dramas from `kisskh.nl`, whose stream and subtitle APIs require a
short-lived `kkey` token. The architecture is organised around **isolating that
authentication problem** so the rest of the pipeline (fetch metadata → resolve
stream URL → download) stays simple.
...
|---|---|
| [`cli.py`](../src/kissget/cli.py) | Click command group and all four subcommands (`dl`, `collect`, `get-key`, `open-browser`). Argument parsing, URL/query resolution, episode-loop orchestration, filename construction. |
| [`kisskh_api.py`](../src/kissget/kisskh_api.py) | `KissKHApi` — HTTP client for the kisskh REST API. Builds endpoint URLs, sends requests with browser-like headers, parses responses into models, and delegates kkey generation to `KkeyProvider`. |
...
| [`manifest.py`](../src/kissget/manifest.py) | `ManifestReader` / `ManifestEpisode` — parse a collector-produced JSON manifest into episode objects for the kkey-free path. |
| [`downloader.py`](../src/kissget/downloader.py) | `Downloader` — downloads video (via N_m3u8DL-RE, falling back to yt-dlp) and subtitles, with optional decryption. Auto-detects binaries. |
